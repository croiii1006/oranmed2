...
export function useSkillsEngine() {
  ...
  // Delay helpers
  const randDelay = () => new Promise<void>(r => { const t = window.setTimeout(r, 800); streamTimers.current.push(t); });
  const subDelay = () => new Promise<void>(r => { const t = window.setTimeout(r, 600); streamTimers.current.push(t); });
  const backendDelay = () => new Promise<void>(r => { const t = window.setTimeout(r, 900); streamTimers.current.push(t); });
  ...
  const restoreState = useCallback((snapshot: SkillsState) => {
    ...
    const ensureSelectMessage = (messages: StreamMessage[]): StreamMessage[] => {
      const exists = messages.some(m => m.type === 'video-gen-status' && m.content.includes('对标视频'));
      if (exists) return messages;
      return [
        ...messages,
        { id: `msg-restore-pick-${Date.now()}`, type: 'video-gen-status' as StreamMessageType, content: '请从右侧面板选择一条对标视频进行复刻 →' },
      ];
    };

    const ensurePromptMessage = (messages: StreamMessage[]): StreamMessage[] => {
      const exists = messages.some(m => m.type === 'video-gen-status' && m.content.includes('Prompt已生成'));
      if (exists) return messages;
      return [
        ...messages,
        { id: `msg-restore-prompt-${Date.now()}`, type: 'video-gen-status' as StreamMessageType, content: '✅ Prompt已生成，请在右侧面板查看和编辑，确认后生成视频 →' },
      ];
    };

    const ensureReadMemoryMessages = (messages: StreamMessage[]): StreamMessage[] => {
    ...
    const ensureFinalMessage = (messages: StreamMessage[]): StreamMessage[] => {
    ...
    const fastForwardToSelect = (base: SkillsState) => {
      ...
      commitState({
        ...
        messages: ensureSelectMessage(
          base.messages.map((m): StreamMessage => {
            if (m.type === 'agent-cluster' && m.agents) {
