...
import { ExternalLink, Sparkles, ShieldCheck, ShieldAlert, ShieldX, Clock, BadgeCheck, AlertCircle, Ban } from 'lucide-react';
...
function PortraitBadge({ status }: { status?: Creator['portraitLicenseStatus'] }) {
...
function AccountBadge({ status }: { status?: Creator['accountStatus'] }) {
  if (!status) return null;
  const map = {
...
export function CreatorDetailDialog({ creator, open, onOpenChange, showMatch }: CreatorDetailDialogProps) {
  ...
  const c = creator;
  const tags = Array.from(new Set([...(c.contentCategories ?? []), ...(c.creatorTags ?? []), ...(c.contentStyleTags ?? [])]));
