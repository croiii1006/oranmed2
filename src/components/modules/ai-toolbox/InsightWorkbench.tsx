...
const GENERATING_PHASES = [
  { label: '生成内容结构', delay: 5000 },
  { label: '提炼竞品差异点', delay: 10000 },
  { label: '整理市场机会判断', delay: 15000 },
  { label: '补充策略建议', delay: 20000 },
  { label: '合成 HTML 结果', delay: 25000 },
];
...
export function InsightWorkbench({ onNavigate }: { onNavigate?: (id: string) => void }) {
  ...
  const { setPrefill: setOranSimPrefill } = useOranSimulationPrefill();
  ...
  const handleJumpToPrediction = useCallback(() => {
    ...
    const attachmentEntries: Array<{ id: string; title: string }> = [];
    if (insightSource) {
      attachmentEntries.push(saveReportMemoryEntry(insightSource, 'insight'));
    }
    if (planningSource) {
    ...
    const prefillBrandName = planningSource?.brandName || insightSource?.brandName || brandName.trim();
    const prefillCategory =
      planningSource?.category ||
      insightSource?.category ||
      category.trim() ||
    ...
    setOranSimPrefill({
      ...
      autoStart: attachmentEntries.length >= 2,
    });
